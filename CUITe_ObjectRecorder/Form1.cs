...
namespace CUITe_ObjectRecorder
{
    public partial class Form1 : Form
    {
        ...
        public void webBrowser1_ClickHandler(object sender, HtmlElementEventArgs ev)
        {
            ...
            {
                if (!sValues.StartsWith("public type "))
                {
            ...
            if(this.btnRecord.Checked == true) 
            {
                ...
                script.SetAttribute("text", @"
                    ...
                    document.onmouseover=function(e){
                        ...
                        if(objInQuestion != null && objInQuestion.id != 'CUITe_div2')
                        {
                            ...
                            if(objInQuestion != null)
                            {
                                ...
                                divTag2.innerHTML = '<span style=\'background:orange;z-index:100;\'><xmp>' + sOutHtml + '</xmp></span>';
                    ...
                    document.onclick=function(e){
                        ...
                        sCode = 'public type var = new type(searchparams)';
                        var sProperties = '';
                        var sNodeName, sNodeType, sId, sName, sClass, sInnerText, sTitle, sValue;
                        if(objInQuestion != null && objInQuestion.id != 'CUITe_div2')
                        {
                            sNodeName   = objInQuestion.nodeName.toLowerCase();
                        ...
                        {
                            if (sName != '') { sProperties += ';Name=' + sName; }
                            if (sClass != '') { sProperties += ';Class=' + sClass; }
                            if (sName != '') { sProperties += ';Name=' + sName; }
                            if (sInnerText != '') { sProperties += ';InnerText=' + sInnerText; }
                            if (sTitle != '') { sProperties += ';Title=' + sTitle; }
                            if (sValue != '') { sProperties += ';Value=' + sValue; }
                        }
                        ...
                        sCode = sCode.replace(' var = ', ' ' + sVarNameEval + ' = ');
                    }
                    function AddDiv2()
                    {
                    ...
                    function getVarName(sInnerText, sTitle, sValue, sName, sId)
                    {
                        ...
                        if (sVarName.length > 25) { vTempCounter++; sVarName = 'Temp' + vTempCounter.toString(); }
                        return sVarName;
                    }
                ...
                string sWindowTitle = (string)doc.InvokeScript("getWindowTitle");
        ...
        private void btnShowCode_Click(object sender, EventArgs e)
        {
            if (this.listBox1.Items.Count > 0)
            {
                ...
                sb.AppendLine("using Microsoft.VisualStudio.TestTools.UITesting;");
                sb.AppendLine("using CUITe.Controls.HtmlControls;");
                sb.AppendLine();
                sb.AppendLine("namespace $ProjectNameSpace$.ObjectRepository");
                sb.AppendLine("{");
                string sWinTitle = (string)doc.InvokeScript("getWinTitForClassName");
                sb.AppendLine("\tpublic class " + sWinTitle + " : CUITe_BrowserWindow");
                sb.AppendLine("\t{");
